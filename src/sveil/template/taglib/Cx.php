...
namespace sveil\template\taglib;

use sveil\template\TagLib;

/**
 * CX标签库解析类
 * @author Richard <richard@sveil.com>
 * @package sveil\template\taglib
 */
class Cx extends Taglib
{
    ...
    public function tagVolist($tag, $content)
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \sveil\Collection || ' . $name . ' instanceof \sveil\Paginator): $' . $key . ' = 0;';
    ...
    public function tagForeach($tag, $content)
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \sveil\Collection || ' . $name . ' instanceof \sveil\Paginator): ';
    ...
    public function tagEmpty($tag, $content)
    {
        $name     = $tag['name'];
        $name     = $this->autoBuildVar($name);
        $parseStr = '<?php if(empty(' . $name . ') || ((' . $name . ' instanceof \sveil\Collection || ' . $name . ' instanceof \sveil\Paginator ) && ' . $name . '->isEmpty())): ?>' . $content . '<?php endif; ?>';
