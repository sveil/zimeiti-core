...
class Url
{
    ...
    protected function parseUrl($url)
    {
        ...
        if (0 === strpos($url, '/')) {
            ...
            if ('' !== $path[0]) {
                $path[0] = null !== $this->app->config('module.' . $path[0]) ? $this->app->config('module.' . $path[0]) : $path[0];
                $url     = implode("/", $path);
            }
        } else {
            // 解析到 模块/控制器/操作
            ...
            if ('' === $module && '' === $controller && '' !== $action) {
                $action = null !== $this->app->config('module.' . $action) ? $this->app->config('module.' . $action) : $action;
            }
