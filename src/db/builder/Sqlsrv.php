...
{
    protected $selectSql       = 'SELECT T1.* FROM (SELECT sveilphp.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS sveilphp) AS T1 %LIMIT%%COMMENT%';
